...
app.post("/api/attendance", (req, res) => {
    ...
    if (clock_in && !clock_out) {
        const checkClockInSql = `
            SELECT * FROM employee_attendance 
            WHERE emp_id = ? AND date = ?
        `;

        db.query(checkClockInSql, [emp_id, today], (err, results) => {
            if (err) {
                console.error(err);
            ...
            const insertSql = `
                ...
                VALUES (?, ?, ?, ?)
            `;
            db.query(insertSql, [emp_id, emp_name, clock_in, today], (err) => {
                if (err) {
                    console.error(err);
    ...
    // ✅ CLOCK OUT
        ...
        `;
        db.query(getClockInSql, [emp_id, today], (err, results) => {
            if (err) {
                console.error(err);
                return res.status(500).json({ error: "Database error when fetching clock in" });
            }
            ...
            db.query(updateSql, [clock_out, total_working_hour, emp_id, today], (err) => {
                if (err) {
                    console.error(err);
                    return res.status(500).json({ error: "Database error when updating clock out" });
                }
    ...
    // ❌ INVALID REQUEST
        ...
        res.status(400).json({ error: "Invalid request: Provide only Clock In or Clock Out" });
    }
});
