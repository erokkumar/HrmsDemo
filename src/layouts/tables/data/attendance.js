...
const API_URL = "http://localhost:5000/api/employee_attendance"; // Replace with your real API
...
, []);
    ...
    const fetchDataEmployee = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/employee_attendance");
        if (!response.ok) throw new Error("Failed to fetch data");
...
useEffect(() => {
  ...
  const updateFilteredData = () => {
    ...
    setFilteredData(
      data.filter((item) => {
        const empMatch = item.emp_name?.toLowerCase().includes(searchLower);
  ...
  const handleCreateClick = () => {
    setModalData({ name: "", leave_type: "", date: "", end_date: "", reason: "", status: "" });
    setIsEditing(false);
    setShowModal(true);
  };
          ...
          <table className="table">
            <thead className="thead-white">
              <tr>
                ...
                <th style={{fontSize:'15px'}}>Clock In</th>
                <th style={{fontSize:'15px'}}>Clock Out</th>
            ...
            <tbody>
              {paginatedData.map((item) => (
                <tr key={item.id}>
                  ...
                  <td style={{fontSize:'15px'}}>{item.emp_name}</td>
                  <td style={{fontSize:'15px'}}>{item.clock_in}</td>
                  <td style={{fontSize:'15px'}}>{item.clock_out}</td>
                  <td style={{fontSize:'15px'}}>{item.total_working_hour}</td>
